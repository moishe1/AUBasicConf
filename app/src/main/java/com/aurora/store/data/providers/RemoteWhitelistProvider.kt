...
class RemoteWhitelistProvider @Inject constructor(
    ...
    suspend fun fetchAndUpdateWhitelist(): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            val phone = Preferences.getString(context, Preferences.PREFERENCE_DEVICE_PHONE_NUMBER)
            val phoneParam = if (phone.isNotEmpty()) {
                "&phone=${URLEncoder.encode(phone, "UTF-8")}"
            } else {
                ""
            }
            val requestUrl = "$remoteWhitelistUrl&id=$deviceId$phoneParam"
            Log.d(TAG, "Fetching whitelist from: $requestUrl")
